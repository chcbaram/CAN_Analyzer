...
bool bootVerifyCrc(void)
{
  ...
  for (uint32_t i=0; i<p_firm_tag->tag_flash_length; i++)
  {
...
void bootCmdReadFirmVersion(cmd_t *p_cmd)
{
  ...
  {
    cmdSendResp(p_cmd, BOOT_CMD_READ_FIRM_VERSION, CMD_OK, (uint8_t *)"empty", 32);
  }
}
...
void bootCmdReadFirmName(cmd_t *p_cmd)
{
  ...
  {
    cmdSendResp(p_cmd, BOOT_CMD_READ_FIRM_NAME, CMD_OK, (uint8_t *)"empty", 32);
  }
}
...
void bootCmdJumpToFw(cmd_t *p_cmd)
{
  if (bootVerifyFw() == true)
  {
    ...
    {
      cmdSendResp(p_cmd, ERR_BOOT_JUMP_TO_FW, ERR_BOOT_FW_CRC, NULL, 0);
    }
  }
  else
  {
    cmdSendResp(p_cmd, ERR_BOOT_JUMP_TO_FW, ERR_BOOT_INVALID_FW, NULL, 0);
  }
}
